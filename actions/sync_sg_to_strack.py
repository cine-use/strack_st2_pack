...
class WriteDataAction(Action):
    ...
    def run(self, json_str):
        st = self.action_service.get_value(name="strack_session")
        if not st:
            st = get_dam_session("strack")
            self.action_service.set_value(name="strack_session", value=st)

        sg = self.action_service.get_value(name="shotgun_session")
        if not sg:
            sg = get_dam_session("shotgun")
            self.action_service.set_value(name="shotgun_session", value=sg)
        ...
        try:
            ...
            if event_meta_data.get("type") == "new_entity":  # start create
                ...
                self.action_service.set_value(name="new_creation", value=new_creation)
            ...
            # if something created, store attributes and create it util attr set done
                ...
                new_creation = self.action_service.get_value(name="new_creation")
            ...
            # do create
            elif self.action_service.get_value(name="in_creating"):  # just created
                new_creation = self.action_service.get_value("new_creation")
